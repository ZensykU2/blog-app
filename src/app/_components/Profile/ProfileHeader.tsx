...
import { Check, X, User, Camera, Settings } from "lucide-react";
...
export function ProfileHeader({ username }: ProfileHeaderProps) {
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      if (pendingDisplayName !== (dbUser.displayName ?? "")) {
        updateBody.displayName = pendingDisplayName;
      }
              ...
              >
                ...
                <p className="text-slate-300 md:text-xl leading-relaxed italic font-light">
                  {dbUser.bio ??
                    (isOwner
                      ? "No story shared yet..."
                      : "This author hasn't shared their story yet.")}
                </p>
              </div>
            )}
          </div>
