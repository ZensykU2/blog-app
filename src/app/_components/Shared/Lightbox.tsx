...
export function Lightbox({ images, initialIndex, onClose }: LightboxProps) {
  ...
  // Properly type‑safe check for custom state
  const isLightboxState = (
    state: unknown
  ): state is { lightboxOpen?: boolean } =>
    !!state && typeof state === "object" && "lightboxOpen" in state;
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (isLightboxState(window.history.state) && window.history.state.lightboxOpen) {
        window.history.back();
      }
    };
  }, [images.length, onClose]);

  // Navigation helpers wrapped in useCallback (for stable deps)
